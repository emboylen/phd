...
academic_artifacts = {
    ...
    'investigate', 'investigated', 'demonstrate', 'demonstrated', 'shown', 'shown',
...
print("✓ N-gram detection complete")
...
print(f"✓ Final vocabulary size: {len(id2word_dictionary)}")

# Create bag-of-words corpus
final_corpus = [id2word_dictionary.doc2bow(text) for text in final_processed_docs]

print(f"✓ Corpus created: {len(final_corpus)} documents")
...
def compute_coherence_values(dictionary, corpus, texts, limit, start=2, step=1):
    ...
    for num_topics in range(start, limit, step):
        print(f"  Training model with k={num_topics} topics...", end='')
        ...
        model = LdaModel(
            ...
            passes=10,             # Total training passes
            iterations=400,        # Iterations per document
        ...
        print(f" Coherence: {coh_score:.4f}")
...
# Find optimal k
x_values = list(range(TOPIC_RANGE_START, TOPIC_RANGE_END, TOPIC_STEP))
optimal_k_index = coherence_values.index(max(coherence_values))
optimal_k = x_values[optimal_k_index]
...
plt.plot(x_values, coherence_values, marker='o', linewidth=2, markersize=8)
...
final_model = model_list[optimal_k_index]
...
<body>
    <div class="container">
        <h1>🔬 Refined Topic Analysis Summary</h1>
...
print(f"✓ Topics summary table saved to: {Path(OUTPUT_TABLE_FILENAME).resolve()}")
...
# ==============================================================================
print("\n" + "=" * 80)
print("✓ REFINED TOPIC MODELING COMPLETE")
print("=" * 80)
print("\nOutput Files:")
...
print(f"  • Coherence (C_v): {optimal_coherence:.4f}")
print(f"  • Final vocabulary: {len(id2word_dictionary)} terms")
